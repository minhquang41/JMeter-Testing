...
HTTP Header Manager
Khai báo các Header cần thiết như:
Content-Type
Authorization
Accept
CSV Data Set Config (nếu sử dụng)
Đọc dữ liệu kiểm thử từ file CSV.
Hỗ trợ nhiều tài khoản hoặc nhiều bộ dữ liệu.
Assertions
...
Trong quá trình kiểm thử, các chỉ số sau được sử dụng để đánh giá hiệu năng hệ thống:
...
Các điểm nghẽn (Bottleneck) trong quá trình xử lý.
Độ ổn định của ứng dụng dưới tải.
Công nghệ sử dụng
Apache JMeter
HTTP/HTTPS Protocol
